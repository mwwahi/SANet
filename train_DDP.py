...
def main():
    ...
    #rank = args.rank  # make sure this is passed as an argument or determined based on your setup
    ...
    #NOTE: `train_collate` would raise an error in an version with newer Torch (2.0+) because the array shape may differ (the input size of images is not all homogeneous) 
...
def train(net, train_loader, optimizer, epoch, writer):
    net.module.set_mode('train') ### NOTE: added `.module.` to call the internal methods of the network
    s = time.time()
    ...
    for j, (input, truth_box, truth_label) in tqdm(enumerate(train_loader), total=len(train_loader), desc='Train %d' % epoch):
        ...
        # print('truth_box.shape',truth_box[0].shape)
        # print('truth_label.shape',truth_label[0].shape)
        # for i, tb in enumerate(truth_box):
        #     print(f"Batch {i} truth_box length: {len(tb)}")

        ### NOTE: heteregenous array shapes will cause an error here in future Torch versions ###
        truth_box = np.array(truth_box)
        truth_label = np.array(truth_label)

        net.module(input, truth_box, truth_label)

        loss, rpn_stat, rcnn_stat = net.module.loss()
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        rpn_cls_loss.append(net.module.rpn_cls_loss.cpu().data.item())
        rpn_reg_loss.append(net.module.rpn_reg_loss.cpu().data.item())
        rcnn_cls_loss.append(net.module.rcnn_cls_loss.cpu().data.item())
        rcnn_reg_loss.append(net.module.rcnn_reg_loss.cpu().data.item())
        ...
        del net.module.rpn_proposals, net.module.detections
        del net.module.total_loss, net.module.rpn_cls_loss, net.module.rpn_reg_loss, net.module.rcnn_cls_loss, net.module.rcnn_reg_loss
        del net.module.rpn_logits_flat, net.module.rpn_deltas_flat

        if net.module.use_rcnn:
            del net.module.rcnn_logits, net.module.rcnn_deltas
