...
class SANet(nn.Module):
    def __init__(self, cfg, mode='train'):
        ...
        self.cfg = cfg
        self.mode = mode
        self.feature_net = FeatureNet(config).cuda()
        self.rpn = RpnHead(config, in_channels=128).cuda()
        self.rcnn_head = RcnnHead(config, in_channels=64).cuda()
        self.rcnn_crop = CropRoi(self.cfg, cfg['rcnn_crop_size']).cuda()
        self.use_rcnn = False
    ...
    def forward(self, inputs, truth_boxes, truth_labels, split_combiner=None, nzhw=None):
        ...
        features, feat_4 = self.feature_net(inputs.to(device))
        fs = features[-1]
